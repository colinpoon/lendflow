 ...
 * Green = strong, Yellow = adequate, Orange = watch, Red = fail.
 */
...
const getRowClasses = (variant: RowVariant): string => {
  switch (variant) {
    case 'total':
      return 'border-t-2 border-gray-400 bg-blue-50/60 hover:bg-blue-50';
    case 'subtotal':
      return 'border-t border-gray-300 bg-gray-50/80 hover:bg-gray-100';
    case 'addback':
      return 'border-b border-gray-100 hover:bg-green-50/30';
    case 'deduction':
      return 'border-b border-gray-100 hover:bg-red-50/20';
    case 'separator':
...
const getLabelClasses = (variant: RowVariant): string => {
  switch (variant) {
    case 'total':
      return 'py-2.5 px-4 font-bold text-gray-900 pl-4';
    case 'subtotal':
      return 'py-2 px-4 font-semibold text-gray-800 pl-4';
    case 'addback':
      return 'py-1.5 px-4 text-gray-600 pl-10 text-sm';
    case 'deduction':
      return 'py-1.5 px-4 text-gray-600 pl-10 text-sm';
    case 'normal':
    default:
      return 'py-2 px-4 text-gray-700 pl-4';
  }
};
...
const getValueClasses = (
  ...
  switch (variant) {
    case 'total':
      return `${base} font-bold text-gray-900 ${colorClass ?? ''}`.trim();
    case 'subtotal':
      return `${base} font-semibold text-gray-800 ${colorClass ?? ''}`.trim();
    case 'addback':
      return `${base} text-sm text-gray-600 ${colorClass ?? ''}`.trim();
    case 'deduction':
      return `${base} text-sm text-gray-600 ${colorClass ?? ''}`.trim();
    case 'normal':
    default:
...
const FinancialTable: React.FC<FinancialTableProps> = ({ data }) => {
  ...
  const renderSectionHeader = (section: SectionConfig) => {
    ...
    if (section.major) {
      return (
        <tr
          className={`${section.collapsible ? 'cursor-pointer hover:bg-slate-100' : ''}`}
          onClick={() =>
        ...
        >
          <td
            ...
            className="py-3 px-4 bg-slate-800 text-white text-xs font-bold uppercase tracking-widest border-t-2 border-slate-700"
          >
    ...
    return (
      <tr
        className={`bg-gray-100 ${section.collapsible ? 'cursor-pointer hover:bg-gray-200' : ''}`}
        onClick={() =>
          section.collapsible && toggleSection(section.title)
        }
      >
          ...
          return (
            <div key={year} className="space-y-3">
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                {/* FCCR equation */}
                {hasFccr && fb && (
                  <div className="bg-gray-50 border border-gray-200 rounded-lg px-5 py-4 font-mono text-sm leading-relaxed space-y-1">
                    <div>
                      <span className="text-gray-500">Numerator</span>
                    ...
                    <div className="border-t border-gray-300 pt-2 mt-1">
                      <span className="text-gray-500">FCCR</span>
                      <span className="text-gray-400"> = </span>
                      <span>
                ...
                {hasSenior && (
                  <div className="bg-gray-50 border border-gray-200 rounded-lg px-5 py-4 font-mono text-sm leading-relaxed">
                    <div>
      ...
      <table className="min-w-full text-sm border-collapse">
        <thead>
          <tr className="bg-slate-800 text-white border-b-2 border-slate-700">
            <th className="py-3 px-4 text-left font-semibold text-slate-200 w-56">
              Line Item
            </th>
            {years.map((y) => (
              <th
                key={y}
                className="py-3 px-4 text-right font-semibold text-slate-200 tabular-nums"
              >
                {y}
              </th>
            ))}
